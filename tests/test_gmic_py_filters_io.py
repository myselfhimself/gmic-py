...
GMIC_FILTERS_SUPPORTED_PARAMETERS = (
    'int', 'float', 'choice', 'bool', 'color', 'text'
...
GMIC_AVAILABLE_INPUT_IMAGES = [
    "sp " + a for a in
    "apples | balloons | barbara | boats | bottles | butterfly | cameraman | car | cat | cliff | chick | colorful | david | dog | duck | eagle | elephant | earth | flower | fruits | gmicky | gmicky_mahvin | gmicky_wilber | greece | gummy | house | inside | landscape | leaf | lena | leno | lion | mandrill | monalisa | monkey | parrots | pencils | peppers | portrait0 | portrait1 | portrait2 | portrait3 | portrait4 | portrait5 | portrait6"
    .split(" | ")
]
...
def is_filter_testable_for_now(filter_dict, already_parsed_filter_commands):
    return filter_dict['lang'] in GMIC_FILTERS_ALLOWED_LANG and filter_dict[
        'command'] not in already_parsed_filter_commands and filter_dict[
            'command'] not in GMIC_FILTERS_TEST_BLACKLIST and is_filter_non_pure_documentation(
                filter_dict)
...
def get_generated_filter_inputs(filter_dict):
    if filter_dict['command'] in GMIC_FILTERS_WITH_SPECIAL_INPUTS:
        if 'input_images_count' in GMIC_FILTERS_WITH_SPECIAL_INPUTS[
                filter_dict['command']]:
            return random.sample(
                GMIC_AVAILABLE_INPUT_IMAGES, GMIC_FILTERS_WITH_SPECIAL_INPUTS[
                    filter_dict['command']]['input_images_count'])
        ...
        if parameter_type not in GMIC_FILTERS_USELESS_PARAMETERS:
            if parameter_type not in GMIC_FILTERS_SUPPORTED_PARAMETERS:
                raise NotImplementedError(
                    'Filter {}\'s {} parameter ({}) must be of supported type {} for now.'
                    .format(filter_dict['command'],
                            (parameter['name'] if 'name' in parameter else
                             '<unnamed>'), parameter['type'],
                            GMIC_FILTERS_SUPPORTED_PARAMETERS))
            if 'default' in parameter:
                generated_parameters_values.append(
                    parameter['default'])  # TODO use fuzzying there?
            else:
                raise NotImplementedError(
                    'Filter {}\'s {} parameter ({}) is one of supported type {} but has no "default" value.'
                    .format(filter_dict['command'],
                            (parameter['name'] if 'name' in parameter else
                             '<unnamed>'), parameter['type'],
                            GMIC_FILTERS_SUPPORTED_PARAMETERS))
    ...
    for category in filters_json_dict['categories']:
        for filter_dict in category['filters']:
            if is_filter_testable_for_now(filter_dict, ids):
                ...
                try:
                    filter_test_params = get_generated_filter_params(
                        filter_dict)
                except NotImplementedError as err:
                    print(err, "Skipped.")
                    continue
                filter_inputs = get_generated_filter_inputs(filter_dict)
                filter_argvalue = (filter_inputs, filter_command,
                                   filter_test_params)

                argvalues.append(filter_argvalue)
                ids.append(filter_command)
            else:
                print("Skipped {}. Not testable or is blacklisted.".format(
                    filter_dict['command']))
                continue
...
def test_gmic_filter_io(filter_inputs, filter_id, filter_params):
    ...
    for pos, param in enumerate(filter_params):
        if type(param) is str and not all(
                isint(param_subelement) or isfloat(param_subelement)
                for param_subelement in param.split(',')) and not isint(
                    param) and not isfloat(param) and not param.endswith(
                        GMIC_OUTPUT_FILE_EXTENSION):
    ...
    gmic_cli_subcommand = "{} -srand {} {} {} output[0] ".format(
        " ".join(filter_inputs), GMIC_FILTERS_RANDOM_SEED, filter_id,
    ...
    gmic_py_command = "{} \"{}\"".format(gmic_py_subcommand,
                                         gmic_py_output_file)
    ...
    gmic_cli_command = "gmic {} \"{}\"".format(gmic_cli_subcommand,
                                               gmic_cli_output_file)
    ...
    assert filecmp.cmp(
        gmic_py_output_file, gmic_cli_output_file,
        shallow=False) or get_images_difference_percent(
            gmic_py_output_file,
            gmic_cli_output_file) <= GMIC_IMAGES_DIFFERENCE_PIL_MAX_PERCENT
